      ...
      <div v-else class="max-h-[2500px] lg:max-h-[804px] overflow-y-auto pr-3 custom-scrollbar">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-4">
          <div
            ...
            :key="item.url"
            class="group bg-white border border-neutral-200/80 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:border-accent-chrono/30 transition-all duration-300 flex flex-col p-3 lg:h-[390px]"
          >
            <!-- Card Thumbnail with Aspect Ratio 1200/630 -->
            <NuxtLink
              :to="`/deconstruct?url=${encodeURIComponent(item.url)}`"
              class="relative w-full aspect-[1200/630] overflow-hidden rounded-xl bg-neutral-50 border border-neutral-100 flex items-center justify-center cursor-pointer"
            >
